...
* classCode 1..1
* classCode = #MANU
  * ^requirements = "SHALL contain exactly one [1..1] @classCode=\"MANU\" Manufactured Product (CodeSystem: HL7RoleClass urn:oid:2.16.840.1.113883.5.110 STATIC) (CONF:81-7900)."
* templateId ^slicing.discriminator.type = #value
  * ^slicing.discriminator.path = "root"
  * ^slicing.rules = #open
* templateId contains primary 1..1
* templateId[primary] ^requirements = "SHALL contain exactly one [1..1] templateId (CONF:81-7901) such that it"
  * root 1..1
  * root = "2.16.840.1.113883.10.20.22.4.37"
    * ^requirements = "SHALL contain exactly one [1..1] @root=\"2.16.840.1.113883.10.20.22.4.37\" (CONF:81-10522)."
* id 1..*
  * ^requirements = "SHALL contain at least one [1..*] id (CONF:81-7902)."
* playingDevice 1..1
  * ^requirements = "SHALL contain exactly one [1..1] playingDevice (CONF:81-7903)."
  * code 0..1
    * ^requirements = "This playingDevice SHOULD contain zero or one [0..1] code (CONF:81-16837)."
* scopingEntity 1..1
  * ^requirements = "SHALL contain exactly one [1..1] scopingEntity (CONF:81-7905)."
  * id 1..*
    * ^requirements = "This scopingEntity SHALL contain at least one [1..*] id (CONF:81-7908)."